...
MON_VCF_ICI
#~ flg, n_samples = get_quality_file_and_n_samples( filename )
d_meta = build_meta_dict( fileName )
Vcf.writer( o_fs, d_meta )
PrintVCFHeader(o_fs,listName,fileName,boolmyname)
...
if ".sam" in fileName: #Checks if it's a stream_file
        while True:
                ...
                #Fills the VCF file
                variant_object.FillVCF(o_fs,nbGeno,table,vcf_field_object)
